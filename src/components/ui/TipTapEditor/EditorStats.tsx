...
export const EditorStats: React.FC<EditorStatsProps> = ({ editor }) => {
  ...
  return (
    <div className="mt-3 px-3 py-2 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-100">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-xs">
        ...
        <div className="flex flex-col items-center justify-center p-2">
          ...
          <div className="text-lg font-bold text-gray-900">{stats.paragraphs}</div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center p-2">
          ...
          <div className="text-lg font-bold text-gray-900">{stats.readingTime}m</div>
        </div>

        {/* Avg Word Length */}
        <div className="flex flex-col items-center justify-center p-2">
          <div className="text-gray-500 font-medium mb-1">Avg Word</div>
          <div className="text-lg font-bold text-gray-900">{stats.avgWordLength}</div>
        </div>
      </div>
        ...
        <div className="mt-3 pt-3 border-t border-blue-100">
          <div className="flex items-center justify-between text-xs">
            ...
            <div className="text-gray-700 font-semibold">
              {((stats.words / (stats.words + stats.paragraphs)) * 100).toFixed(0)}%
            </div>
          </div>
          <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
            <div
              ...
              style={{
                width: `${Math.min(((stats.words / (stats.words + stats.paragraphs)) * 100), 100)}%`,
              }}
            />
          </div>
        </div>
