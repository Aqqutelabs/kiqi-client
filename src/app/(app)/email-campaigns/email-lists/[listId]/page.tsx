    ...
    <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 flex flex-col">
      ...
      <div className="bg-white rounded-lg shadow-md flex-1 flex flex-col">
        <div className="p-4 border-b flex flex-wrap gap-4 justify-between items-center">
          ...
          {uploading && (
            <div className="text-blue-600 text-xs mt-2">Uploading CSV...</div>
          )}
          {uploadError && (
            <div className="text-red-500 text-xs mt-2">{uploadError}</div>
          )}
          {uploadSuccess && (
            <div className="text-green-600 text-xs mt-2">{uploadSuccess}</div>
          )}
          <div className="flex items-center gap-2">
            <Input
              icon={<Search size={16} />}
            ...
            <Button
              variant="tertiary"
              className="!bg-white border !text-gray-700 h-9">
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-100/70">
                <tr className="text-left text-gray-600">
                  {["Email Address", "Full Name", "Date Added", "Action"].map(
                    (h) => (
